...
export default function ModernFirstOtp() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const params = new URLSearchParams(searchParams);
  const [time, setTime] = useState(180);
    ...
    toast.promise(
      fetch(`${process.env.API_URL || ""}/api/order/${params.get("id")}`, {
        ...
        .then((data) => {
          if (data && data._id) {
            setTimeout(() => {
              router.push(`/birth-place?id=${data._id}`);
            }, 1000);
            return data;
          } else {
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-slate-800">
          ...
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400">Enter a verification code</p>
            </div>
            ...
            <div className="p-4 bg-slate-800/30 rounded-xl border border-slate-700">
              <p className="text-sm text-gray-400 flex items-start gap-2">
                ...
                <span>
                  <strong className="text-teal-400">Security Tip:</strong> Never
                  share your OTP code with anyone. Our team will never ask for
                  your OTP.
                </span>
              </p>
            </div>
